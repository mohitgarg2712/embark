  ...
  async.waterfall([
    ...
    function listAccounts(next) {
      self.runCommand(self.client.listAccountsCommand(), {}, (err, stdout, _stderr) => {
        ...
        if(addresses.length >= this.config.accounts.numAccounts){ 
          return next(ALREADY_INITIALIZED);
        }
        // pass in the numAccounts we need to be created
        next(null, this.config.accounts.numAccounts - addresses.length);
      });
    },
    function genesisBlock(next, numAccountsToBeCreated) {
    ...
    function newAccount(next, numAccountsToBeCreated) {
      for(let i=0; i<(numAccountsToBeCreated || 1); i++){
        self.runCommand(self.client.newAccountCommand(), {}, (err, stdout, _stderr) => {
          if (err) {
            return next(err);
          }
          addresses.push(stdout.match(/{(\w+)}/gm));
        });
      }
      next();
    }
