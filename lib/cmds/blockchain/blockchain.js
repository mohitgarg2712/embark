...
var Blockchain = function(options) {
  ...
  console.dir('[blockchain/blockahin]: blockahin config = ' + JSON.stringify(this.config));
...
Blockchain.prototype.generateAccounts = function(numAccountsToCreate, callback){
  const self = this;
  async.times(numAccountsToCreate, (_n, cb) => {
    self.runCommand(self.client.newAccountCommand(), {}, (err, stdout, _stderr) => {
      console.dir('[blockchain/blockchain] : err: ' + err + ', stdout: ' + stdout + ', stderr: ' + _stderr);
      if (err) {
        return cb(err);
      }
      cb(null, stdout.match(/{(\w+)}/)[1]);
    });
  }, function(err, addresses){
    console.dir('[blockchain/blockchain]: done aded new accts: ' + JSON.stringify(addresses) + ', hasError: ' + err);
...
Blockchain.prototype.run = function() {
  ...
  async.waterfall([
    ...
    function init(next) {
      if (!self.isDev || self.config.account.numAccounts) {
        return self.initChainAndGetAddresses((err, addrs) => {
          console.dir('[BLOCKCHAIN/BLOCKCHAIN]: got address from initChainAndGetAddresses: ' + JSON.stringify(addrs));
          addresses = addrs;
          next(err);
        });
      }
    ...
    function getMainCommand(next) {
      self.client.mainCommand(addresses, function(cmd, args) {
        next(null, cmd, args);
      }, true);
    }
...
Blockchain.prototype.initChainAndGetAddresses = function(callback) {
  const self = this;
  let addresses = [];
  ...
  async.waterfall([
    ...
    function listAccounts(next) {
      self.runCommand(self.client.listAccountsCommand(), {}, (err, stdout, _stderr) => {
        console.dir('[blockchain/blockchain]: LIST ACCOUNTS');
        if ((err || stdout === undefined || stdout.match(/{(\w+)}/) === null || stdout.indexOf("Fatal") >= 0) && !self.config.account.numAccounts) {
          console.log(__("no accounts found").green);
          return next();
        }
        console.log(__("already initialized").green);
        addresses = stdout.match(/{(\w+)}/gm) || [];
        addresses = addresses.map((addr) => { return addr.replace('{', '').replace('}', ''); });
        // if the number of accounts the node controls is already >=
        // the number of accounts we want to create, then node can
        // be considered already initialized
        console.dir('[blockchain/blockchain]: addresses.length: ' + addresses.length + ', self.config.account.numAccounts: ' + self.config.account.numAccounts);
      
        if(addresses.length >= self.config.account.numAccounts){ 
          return next(ALREADY_INITIALIZED);
        }
        // pass in the numAccounts we need to be created
        const numAccountsNeeded = (self.config.account.numAccounts - addresses.length) || 1;
        addresses = self.generateAccounts(numAccountsNeeded, function(err, addressesCreated){
    ...
    // function genesisBlock(numAccountsToBeCreated, next) {
    //   if (!self.config.genesisBlock) {
    //     return next(null, numAccountsToBeCreated);
    //   }
    //   console.log(__("initializing genesis block").green);
    //   self.runCommand(self.client.initGenesisCommmand(), {}, (err, _stdout, _stderr) => {
    //     next(err, numAccountsToBeCreated);
    //   });
    // },
    // function newAccount(numAccountsToBeCreated, next) {
    //   self.runCommand(self.client.newAccountCommand(), {}, (err, stdout, _stderr) => {
    //     if (err) {
    //       return next(err);
    //     }
    //     addresses.push(stdout.match(/{(\w+)}/));
    //   });
    //   next();
    // }
  ], (err) => {
