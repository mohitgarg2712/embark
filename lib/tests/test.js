...
class Test {
  ...
  initWeb3Provider(callback) {
    ...
    if (this.simOptions.host) {
      ...
      console.log("Init again with sim");
      return utils.pingEndpoint(host, port, type, protocol, this.engine.config.blockchainConfig.wsOrigins.split(',')[0], (err) => {
        if (err) {
          console.error(`Error connecting to the node, there might be an error in ${host}`.red);
          return callback(err);
        }

        self.provider = self.buildProvider(protocol, host, port, type, accounts);
    ...
    let {host, port, type, protocol} = this.engine.config.contractsConfig.deployment || {host: "127.0.0.1"};
    if (!protocol) {
    ...
    return utils.pingEndpoint(host, port, type, protocol, this.engine.config.blockchainConfig.wsOrigins.split(',')[0], (err) => {
      if (err) {
        if (!self.sim) {
          self.sim = getSimulator();
        }
        self.web3.setProvider(self.sim.provider(self.simOptions));
        this.subscribeToPendingTransactions();
        return callback();
      }

      self.provider = self.buildProvider(protocol, host, port, type);
      return self.provider.startWeb3Provider((err) => {
        if (err) {
          return callback(err);
        }
  ...
  buildProvider(protocol, host, port, type, accounts) {
  ...
  init(callback) {
    ...
    this.engine.init({logger: new TestLogger({logLevel: this.options.loglevel})});
