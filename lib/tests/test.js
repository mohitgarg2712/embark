...
class Test {
  ...
  initWeb3Provider(callback) {
    ...
    if (!this.connected) {
      const deploymentConfig = this.engine.config.contractsConfig.deployment;
      const web3Endpoint = utils.buildUrl(deploymentConfig.protocol, deploymentConfig.host, deploymentConfig.port);
      let provider;

      if (deploymentConfig.type === 'ws') {
        provider = this.web3.providers.WebsocketProvider;
      } else {
        provider = this.web3.providers.HttpProvider;
      }
